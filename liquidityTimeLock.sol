...
contract liquidityTimeLock is Context ,Ownable{
    ...
    function lockLiquidity(address _pairAddress ,address token1 , address token2  ,  uint256 amount ,  uint256 periodInHR , address _reciever ) public payable{
    ...
    bool inArray;
       ...
       newlplock.unlockTime = block.timestamp + (periodInHR * 1 hours);
    ...
    function _filterPair(address token1 , address token2) private {
    ...
    function addLpPair(address pairAddress , address token1 , address token2) private {
        if(!LPtokens[pairAddress].isSet){
          ...
          newLpToken.token1 = token1;
          newLpToken.token2 = token2;
    ...
    function processPayment(address _pairAddress ,uint256 amount) private  returns (bool){
        ...
        IBEP20 currentPair = IBEP20(_pairAddress);
    ...
    // function deductFees(address _pairAddress,uint256 amount) private returns(uint256){
    //     if(lpFees > 0){
    //         uint256 fees = amount * lpFees / 100;
    //         LPtokens[_pairAddress].lpfees = LPtokens[_pairAddress].lpfees + fees;
    //         return amount - fees;
    //     }
    //     return amount;
    // }
    ...
    function _unLockLiquidity(address _pairAddress , uint256 lockID ) private returns(bool){
        ...
        IBEP20 token = IBEP20(_pairAddress);
    ...
    function getPairAddress(address token1 , address token2) public  view returns(address){
        return  IUniswapV2Factory(SwapRouter.factory()).getPair(token1, token2);
    }
    function getTokens(address _pairAddress) public view returns(address token1 ,address token2){
        IUniswapV2Pair pair = IUniswapV2Pair(_pairAddress);
         token1 = pair.token0();
         token2 = pair.token1();
        require(token1 != address(0) || token2 != address(0) , "invalid lp pair address");
